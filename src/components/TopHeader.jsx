...
const TopHeader = ({ title, userData, allClasses }) => {
  ...
  const isAdmin =
    ...
    userData?.role === "מנהל" ||
    userData?.role === "admin";
  const isRegularUser = !isInstructor && !isAdmin;

  useEffect(() => {
    const normalizePhone = (phone) => phone?.replace(/\D/g, "");

    const fetchStats = async () => {
      if (!isInstructor || !userData?.phone || !Array.isArray(allClasses))
        return;
      ...
      try {
        ...
        const uniqueUserIds = [
          ...new Set(relevantBookings.map((b) => b.userId)),
        ];
  ...
  useEffect(() => {
    const fetchUserStats = async () => {
      try {
        ...
        const instructors = nonAdminUsers.filter(
          (u) => u.isInstructor === true
        );
        const regularUsers = nonAdminUsers.filter(
          (u) => u.isInstructor !== true
        );
    ...
    if (isAdmin) fetchUserStats();
  }, [isAdmin]);
  console.log("userData.role", userData?.role);
console.log("userData.isInstructor", userData?.isInstructor);
...
{userData && isRegularUser && (
  <div
    className={`${
      userData.remainingLessons === 0 ? "bg-red-500" : "bg-green-600"
    } text-white font-semibold py-1 px-3 rounded-lg text-sm select-none whitespace-nowrap`}
  >
    נשארו לך {userData.remainingLessons ?? 0} מתוך {userData.weeklyLimit ?? 0} שיעורים
  </div>
)}
