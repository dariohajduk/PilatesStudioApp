...
const AdminHealthDeclarations = ({ employee }) => {
  ...
  useEffect(() => {
    ...
    const fetchUsersAndDeclarations = async () => {
      try {
        // שלב 1: טען את כל המשתמשים שאינם מנהלים או מדריכים
        const usersSnapshot = await getDocs(collection(db, "Users"));
        const users = usersSnapshot.docs
          .map(doc => ({ id: doc.id, ...doc.data() }))
          .filter(user => !user.isAdmin && !user.isInstructor);

        // שלב 2: טען את כל העובדים עם הצהרה חתומה
        ...
        // שלב 3: השוואה לפי טלפון (user.phone === employee.id)
        const signed = users
        .map(user => {
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex justify-center gap-6 mb-4 text-sm md:text-base">
        <div className="bg-green-100 text-green-800 px-4 py-2 rounded shadow">
          הצהרות חתומות: <strong>{signedUsers.length}</strong>
        </div>
        <div className="bg-red-100 text-red-800 px-4 py-2 rounded shadow">
          הצהרות חסרות: <strong>{unsignedUsers.length}</strong>
        </div>
      </div>
      ...
      <div className="text-center mb-6">
        ...
        >
          {showUnsigned ? "הצג הצהרות חתומות" : "הצג הצהרות חסרות"}
        </button>
      </div>
      ...
      {displayedUsers.length === 0 ? (
        <p className="text-center text-gray-500">לא נמצאו תוצאות מתאימות.</p>
      ) : (
...
>
  ...
  {showUnsigned ? (
    <p className="text-red-600 mt-2">אין הצהרת בריאות חתומה</p>
  ) : (
    <>
