  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const userData = {
        ...
        preferredTimeRange: preferredStartTime && preferredEndTime 
...
const registerToMatchingClasses = async (user) => {
  try {
    ...
    console.log(`📆 נמצאו ${futureClasses.length} שיעורים עתידיים עם מקומות פנויים`);
    ...
    if (isWeeklySubscription || isMonthlySubscription) {
      ...
      if (isWeeklySubscription) {
        console.log(`📊 שיעורים עתידיים מחולקים ל-${Object.keys(classesByWeek).length} שבועות`);
        console.log(`📊 הרשמות קיימות לפי שבוע:`, bookingsByWeek);
      }
      
      if (isMonthlySubscription) {
        console.log(`📊 שיעורים עתידיים מחולקים ל-${Object.keys(classesByMonth).length} חודשים`);
    ...
    if (user.preferredDays && user.preferredDays.length > 0 && user.preferredTimeRange) {
      ...
      for (const cls of futureClasses) {
        try {
          ...
          if (isWeeklySubscription) {
            ...
            const existingWeeklyBookings = (bookingsByWeek[weekKey] || []).length;
            const newWeeklyBookings = (weeklyRegistrations[weekKey] || 0);
            const totalWeeklyBookings = existingWeeklyBookings + newWeeklyBookings;
            
            // אם כבר הגענו למספר המקסימלי של שיעורים בשבוע זה, דלג
            if (totalWeeklyBookings >= user.remainingLessons) {
              console.log(`⚠️ הגענו למקסימום שיעורים בשבוע ${weekKey}: ${totalWeeklyBookings}/${user.remainingLessons}`);
              continue;
            }
          }
          ...
          if (isMonthlySubscription) {
            ...
            const existingMonthlyBookings = (bookingsByMonth[monthKey] || []).length;
            const newMonthlyBookings = (monthlyRegistrations[monthKey] || 0);
            const totalMonthlyBookings = existingMonthlyBookings + newMonthlyBookings;
            
            // אם כבר הגענו למספר המקסימלי של שיעורים בחודש זה, דלג
            if (totalMonthlyBookings >= user.remainingLessons) {
              console.log(`⚠️ הגענו למקסימום שיעורים בחודש ${monthKey}: ${totalMonthlyBookings}/${user.remainingLessons}`);
              continue;
            }
          }
          
          if (isCardSubscription && registrationCount >= user.remainingLessons) {
            console.log(`⚠️ הגענו למקסימום שיעורים בכרטיסייה: ${registrationCount}/${user.remainingLessons}`);
          ...
          console.log(`בדיקת שיעור: ${cls.name}, תאריך: ${cls.date}, יום: ${dayOfWeek}, שעה: ${cls.time}`);
          ...
          if (user.preferredDays.includes(dayOfWeek)) {
            ...
            const [classHours, classMinutes] = cls.time.split(':').map(Number);
            ...
            if (classTimeInMinutes >= startMinutes && classTimeInMinutes <= endMinutes) {
              ...
              if (existingBookings.empty) {
                ...
                console.log(`✅ נרשם לשיעור: ${cls.name}, ${cls.date}, ${cls.time}`);
                ...
                if (isWeeklySubscription) {
                  ...
                  console.log(`📊 הרשמות בשבוע ${weekKey}: ${weeklyRegistrations[weekKey]} מתוך ${user.remainingLessons}`);
                }
                
                if (isMonthlySubscription) {
                  monthlyRegistrations[monthKey] = (monthlyRegistrations[monthKey] || 0) + 1;
                  console.log(`📊 הרשמות בחודש ${monthKey}: ${monthlyRegistrations[monthKey]} מתוך ${user.remainingLessons}`);
                }
                ...
                if (registrationCount >= maxTotalRegistrations) {
                  console.log(`🛑 הגענו למקסימום רישומים אוטומטיים כללי: ${registrationCount}`);
                  break;
                }
              } else {
      ...
      if (registrationCount > 0) {
        ...
        if (isCardSubscription) {
          ...
          setMessage(`✅ המשתמש נרשם אוטומטית ל-${registrationCount} שיעורים ונותרו לו ${user.remainingLessons - registrationCount} שיעורים בכרטיסייה`);
        } 
        else if (isWeeklySubscription) {
          setMessage(`✅ המשתמש נרשם אוטומטית ל-${registrationCount} שיעורים (מנוי שבועי - מוגבל ל-${user.remainingLessons} שיעורים בשבוע)`);
        }
        else if (isMonthlySubscription) {
          setMessage(`✅ המשתמש נרשם אוטומטית ל-${registrationCount} שיעורים (מנוי חודשי - מוגבל ל-${user.remainingLessons} שיעורים בחודש)`);
        }
      ...
      console.log(`❌ חסרים פרטי העדפות: ימים=${user.preferredDays?.length || 0}, טווח שעות=${user.preferredTimeRange || 'חסר'}`);
...
const formatDateToIL = (date) => {
  ...
  const checkAdmin = () => {
    // לוג לדיבוג
    console.log("פרטי המשתמש:", employee);
    ...
    console.log("האם מנהל:", isUserAdmin, 
                ...
                {dayNames.map((day, index) => (
                  ...
                  >
                    <input
                      ...
                      className="hidden"
                    />
                    {preferredDays.includes(index) && <Check size={14} className="mr-1" />}
                    <span>{day}</span>
                  </label>
                ))}
              </div>
              ...
              <div className="flex flex-col text-right">
                ...
                {user.preferredDays && user.preferredDays.length > 0 && (
                  <div className="text-xs text-gray-500 mt-1">
                    <span className="font-medium">ימים מועדפים:</span> {user.preferredDays.map(day => dayNames[day]).join(', ')}
                    {user.preferredTimeRange && ` | שעות: ${user.preferredTimeRange}`}
                    {user.autoJoin && <span className="text-green-600 mr-1"> • רישום אוטומטי</span>}
                  </div>
                )}
              </div>
